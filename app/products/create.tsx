...
// ─── ProductAttrCard — module-level so re-renders don't unmount it ─────────
...
export default function CreateProductScreen() {
  ...
  useEffect(() => {
    Promise.all([api.getCategories(), api.getAttributes()]).then(([cRes, aRes]) => {
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      ...
      const t = asset.type === 'image' ? 'image/jpeg' : asset.mimeType ?? 'image/jpeg';
      setImage({ uri: asset.uri, name: n, type: t });
    }
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      <View className="bg-white border-b border-gray-100 px-4 py-3 flex-row items-center gap-3">
        ...
        <Text className="font-semibold text-base text-gray-900">New Product</Text>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        <View className="px-4 py-4 gap-5">
          ...
          <TouchableOpacity onPress={pickImage} className="items-center">
            ...
            <Text className="text-xs text-gray-400 mt-2">Tap to add photo (max 5MB)</Text>
          </TouchableOpacity>
